...
for subdir, dirs, files in tqdm(list(os.walk(rootdir)), position=0, leave=True):
    for file in files:
        exclude_list = ["trumbowyg", "logtailer", "ace",
                        "migrations", "git-hooks", "cronjob_scripts", "DBMigrations", "VersionCompatibilityScripts"]
...
pep8check = tqdm(position=0, leave=True, total=5)
...
livechatapp_tests_check = subprocess.run(
    "flake8 --ignore=" + whitelisted_flake8_codes + ",N806,F811,F841" + " ./LiveChatApp/tests/*.py", shell=True)
...
global_file_check1 = subprocess.run(
    "flake8 --ignore=" + whitelisted_flake8_codes + " *.py", shell=True)

global_file_check2 = subprocess.run(
    "flake8 --ignore=" + whitelisted_flake8_codes + " */*.py", shell=True)

global_file_check3 = subprocess.run(
    "flake8 --ignore=" + whitelisted_flake8_codes + " */*/*.py", shell=True)
...
pep8formattingcheck = val1.returncode + easychatapp_check.returncode + \
    ...
    easytmsapp_tests_check.returncode + \
    livechatapp_tests_check.returncode + easyassistapp_check.returncode + \
    global_file_check1.returncode + global_file_check2.returncode + global_file_check3.returncode
...
if (pep8formattingcheck + val5 + commit_changes_count_check + val9) != 0:
