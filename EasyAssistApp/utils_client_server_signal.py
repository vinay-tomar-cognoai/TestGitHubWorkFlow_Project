...
def send_data_from_server_to_client(function_name, response, user):
    ...
    try:
        ...
        if settings.DEBUG:
            uri = "ws://127.0.0.1:8000/ws/cognoai-signal/" + str(websocket_token) + "/server/"
        else:
            uri = "wss://" + settings.EASYCHAT_DOMAIN + "/ws/cognoai-signal/" + str(websocket_token) + "/server/"
        ...
        logger.error("send_data_from_server_to_client: " + str(e) +
                     " at " + str(exc_tb.tb_lineno), extra={'AppName': 'EasyAssist'})
