...
function sync_client_web_screen(e) {
    ...
    if (message.header.sender == "client") {
        ...
        if (client_packet.type == "html") {
            ...
            set_client_response(client_packet.message, client_packet.attachment, client_packet.attachment_file_name, client_packet.name, window.ADMIN_AGENT_USERNAME);
        ...
        if(window.IS_AGENT == "True"){
            ...
            if (client_packet.type == "sync-scroll") {
                ...
                show_easyassist_toast("Customer asked for edit access to admin agent.");
            } else if (client_packet.type == "chat") {
                // I am invited agent && and received message is not mine but from another invited agent
                if(window.INVITED_AGENT_NAME != client_packet.agent_name){
                    set_client_response(client_packet.message, client_packet.attachment, client_packet.attachment_file_name, client_packet.agent_name, client_packet.sender);
                }
            } else if (client_packet.type == "livechat-typing"){
            ...
            if (client_packet.type == "chat") {
                ...
                if(window.CLIENT_NAME != client_packet.agent_name){
                    set_client_response(client_packet.message, client_packet.attachment, client_packet.attachment_file_name, client_packet.agent_name, client_packet.sender);
                }
...
function join_meeting_invited_agent(){
    if(ENABLE_VOIP_WITH_VIDEO_CALLING == "True") {
...
function save_cobrowsing_chat_history(chat_message, sender, attachment, attachment_file_name, sender) {
...
function send_chat_message_from_agent(message, attachment, attachment_file_name) {
      if(window.IS_AGENT == "False"){
          save_cobrowsing_chat_history(message, "agent", attachment, attachment_file_name, "client");
      }
...
function set_client_response(message, attachment, attachment_file_name, name, sender) {
    ...
    if(window.IS_AGENT == "False"){
        save_cobrowsing_chat_history(message, "client", attachment, attachment_file_name, sender);
    }
}
...
function remove_easyassist_blur_fields(){
    try {
        ...
        var obfuscated_elements = frameDocument.querySelectorAll("[easyassist-obfuscate]")
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if(window.location.href.indexOf('?') > 0){
                window.location.href = window.location.href + "&session_ended=true";
            } else {
                window.location.href = window.location.href + "?session_ended=true";
            }
