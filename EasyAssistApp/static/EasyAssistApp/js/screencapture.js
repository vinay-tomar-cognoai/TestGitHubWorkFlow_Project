...
function easyassist_time_to_show_support_button() {
    ...
    if(EASYASSIST_COBROWSE_META.agent_working_days.indexOf(current_day) == -1) {
        return false;
    }
...
window.addEventListener('message', function(event) {
    ...
    if (~event.origin.indexOf(EASYASSIST_HOST_PROTOCOL + "://" + EASYASSIST_COBROWSE_HOST)) {
        ...
        if(event.data.event == "voip_function_error") {
            easyassist_show_function_fail_modal(event.data.error_code);
        }
...
(function(exports) {
    ...
    window.easyassist_total_input_elements = 0;
    window.weak_connection_cobrowsing_interval = null;
    ...
    function easyassist_set_value_attr_into_screenshot(screenshot) {
        ...
        for(let index = 0; index < screenshot_custom_select_elements.length; index++) {
            let is_obfuscated = easyassist_obfuscated_element_check(screenshot_custom_select_elements[index]);
            if(is_obfuscated[0])
    ...
    function easyassist_update_visited_node_map(dom_node) {
        try {
            ...
            let easyassist_element_id = dom_node.getAttribute('easyassist-element-id');
    ...
    function easyassist_check_weak_connection_enabled() {
        if(EASYASSIST_COBROWSE_META.enable_low_bandwidth_cobrowsing && AGENT_WEAK_CONNECTION_DETECTED) {
            return true;
        }
        return false;
    }

    function easyassist_get_label_from_child_elements(parent_element) {
        var element_label = "";
    ...
    function easyassist_get_input_element_label(element) {
        ...
        var element_text_not_consider = ["select", "li", "ul", "input", "button", "textarea"];

        while(level > 0) {
            var parent_element_children = parent_element.children;
            for(let idx = 0; idx < parent_element_children.length; idx ++) {
                ...
                if(element_text_not_consider.indexOf(child_tag_name) == -1) {
                    element_label += easyassist_get_label_from_child_elements(parent_element_children[idx]);
                }
            }
            ...
            while(level > -1) {
                ...
                for(let idx = 0; idx < parent_element_children.length; idx ++) {
                    if(parent_element_children[idx] == element) {
                        ...
                        if(element_text_not_consider.indexOf(child_tag_name) == -1) {
                            element_label += (parent_element_children[idx].innerText || "").trim()
                        }
                    }
                }
    ...
    function easyassist_get_weak_connection_html() {

        // var html_element_stack = [document.body];

        var visited_tag_map = {}
        var html_element_list = easyassist_get_input_element();

        let easyassist_total_input_elements = html_element_list.length;
        var html_data = "";
        ...
        for(let idx = html_element_list.length - 1; idx >= 0; idx --) {
            ...
            if(child_element_tag == "button" || child_element_type == "button" || child_element_type == "submit") {
                ...
                if(child_element_name && (child_element_name in visited_tag_map)) {
                    continue;
                }
                visited_tag_map[child_element_name] = true;
                var radio_elements = document.querySelectorAll("[name='" + child_element_name + "']");
                var radio_html = "";
                var radio_label = easyassist_get_input_element_label(child_element.parentElement);
                ...
                let child_label = easyassist_get_input_element_label(child_element);
        ...
        var screenshot_links = screenshot.querySelectorAll("link");
        for(let idx = 0; idx < screenshot_links.length; idx ++) {
            screenshot_links[idx].parentNode.removeChild(screenshot_links[idx]);
        }

        var screenshot_scripts = screenshot.querySelectorAll("script");
        ...
        var screenshot_body = screenshot.querySelector("body");
        ...
        var outerhtml = screenshot.outerHTML;
    ...
    function easyassist_get_input_element() {
        ...
        while(html_element_stack.length) {
            ...
            for(let idx = 0; idx < current_element_children.length; idx ++) {
                ...
                if(easyassist_check_input_element_is_hidden(child_element)) {
    ...
    function easyassist_screenshot_page() {
        ...
        // console.log("screenshot_page : ", easyassist_check_weak_connection_enabled());

        if(easyassist_check_weak_connection_enabled()) {
            return easyassist_get_weak_connection_html();
        }
        ...
        var screenshot_iframes = screenshot.querySelectorAll("iframe");
        for(let index = 0; index < screenshot_iframes.length; index++) {
            screenshot_iframes[index].parentNode.removeChild(screenshot_iframes[index]);
        }
    ...
    function easyassist_add_request_assist_modal() {
        ...
        for(let idx = 0; idx < verify_inputs.length; idx ++) {
            ...
            verify_inputs[idx].addEventListener('keydown', function(e) {
                ...
                } else if(e.key == "ArrowDown" || e.key == "ArrowUp" || e.key == "-" || e.key == "_" || e.key == "+" || e.key == "=" || e.key == "e" || e.key == "E") {
                    e.preventDefault();
                    e.stopPropagation();
                }
            });
    ...
    function easyassist_add_floating_sidenav_menu() {
        ...
        if(EASYASSIST_COBROWSE_META.is_mobile == true){
            sidenav_menu = '\
            <div class="easyassist-custom-nav-bar_wrapper" id="easyassist-sidenav-menu-id" style="display:none;">\
    ...
    function easyassist_create_close_nav_timeout(){
        if(easyassist_close_nav_timeout == null){
            let easyassist_close_nav_timeout = setTimeout(easyassist_close_sidenav, 15*1000);
        }
    }
    ...
    function easyassist_check_input_element_is_hidden(element) {
        var element_computed_style = getComputedStyle(element);
        if(element_computed_style.display == "none") {
            return true;
        }
    ...
    function easyassist_apply_custom_select(document_select_tag_list) {
       ...
       let easyassist_custom_dropdown_applied = true;
    }
    ...
    function easyassist_display_masked_field_warning(event){
        ...
        var warning_element =  easyassist_get_eles_by_class_name("easyassist-tooltiptext")[0];
    ...
    function easyassist_remove_masked_field_warning(){
        easyassist_get_eles_by_class_name("easyassist-tooltiptext")[0].style.visibility = "hidden";
        easyassist_get_eles_by_class_name("easyassist-tooltiptext")[0].style.opacity = "0";
    }

    function easyassist_get_node_visited_status(dom_node) {
        var easyassist_element_id = dom_node.getAttribute("easyassist-element-id");
        if(easyassist_visited_nodes_map[easyassist_element_id]) {
            if(dom_node.className != easyassist_visited_nodes_map[easyassist_element_id].class) {
                return "class_change";
    ...
    exports.easyassist_get_input_element = easyassist_get_input_element;
    exports.easyassist_check_weak_connection_enabled = easyassist_check_weak_connection_enabled;
    exports.easyassist_start_parse_dom_tree_interval = easyassist_start_parse_dom_tree_interval;
    exports.easyassist_create_value_attr_into_document = easyassist_create_value_attr_into_document;
    ...
    function easyassist_start_parse_dom_tree_interval() {
        if(parse_dom_tree_interval != null) {
            return;
        }

        if(EASYASSIST_COBROWSE_META.allow_screen_sharing_cobrowse == true) {
            if(parse_dom_tree_interval != null) {
