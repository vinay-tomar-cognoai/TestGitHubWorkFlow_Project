...
window.onload = function(e){
    ...
    }else if(enable_agent_connect_message == "False"){
...
function append_bot_text_response(text_response, agent_name = null,time = null,agent_profile_pic_source){
        if(time == null)
            var time = return_time();
        let profile = "";
        let msg_sender_icon = ""
            if (DISPLAY_AGENT_PROFILE == "True" && agent_profile_pic_source != "") {
            ...
            if (agent_to_customer_cobrowsing == "True") {
                if (profile != "") {
                    ...
                    msg_sender_icon = ['<svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">',
            ...
            if (agent_to_customer_cobrowsing == "True") {
                var html = ['<div class="easychat-bot-message-div">',
                    ...
                    '<div class="easychat-bot-message-name" style="color: ' + BOT_THEME_COLOR + '">' + agent_name + '</div>',
        ...
        setTimeout(function(e){reset_size_of_text_field();}, 200);
}

function reset_size_of_text_field(){
    var viewmore_height = document.getElementsByClassName('easychat-show-less-text')[document.getElementsByClassName('easychat-show-less-text').length-1].scrollHeight;
...
function reset_size_of_text_field_all(){
    ...
    for(let index=0 ; index<elements.length ; index++){
        viewmore_height = elements[index].scrollHeight
        if(viewmore_height>300){
...
function append_captcha(url){
    var captcha_element = document.getElementById("recaptcha-div");
    if(captcha_element!=null || captcha_element!=undefined){
        captcha_element.remove();
    }
    var captcha_html = '<div class="easychat-captcha-div" id="recaptcha-div" align="center">\
...
function append_google_map_location(){
    ...
    var location_html = '<div class="easychat-gmap-wrapper" id="google-map">\
...
function append_bot_response(response){
    ...
    if(response.is_attachment_required ==true){
      ...
      var choosen_file_ext = file_type_ext[choosen_file_type];
...
function append_bot_slider_images(image_url_list){
    ...
    var image_slidershow_html = '<div class="easychat-slider-wrapper">\
    <div style="color:'+BOT_THEME_COLOR+'; width: 80%;margin: auto;"><div class="slideshow-container"  value=1 >';
    var total_images = image_url_list.length;

    for(let i=0;i<image_url_list.length;i++){
        var current_image_no = i+1;
        if (i==0){
            ...
            if(el.files[0] != undefined || el.files[0] != null){
                ...
                if(document.getElementById("easychat-uploadfile__XPS").getAttribute("accept").toString().indexOf(file_ext.toLowerCase()) !== -1){
                    if(el.files[0].size  <=  5 * 1024 * 1024){
                        ...
                        var file_name = el.files[0].name.split(".")
    ...
    var tables_html = '<!--<style>table {border-left: 0;border-spacing: 0px;box-shadow: 1px 1px 6px rgba(0, 0, 0, 0.2);background-color: white;}th,td {    padding: 5px 4px 6px 4px; text-align: left; vertical-align: top;}tr {display: table-row;    vertical-align: inherit;    border-color: inherit;}td {    border-top: 1px solid #000;word-break: break-all;    min-width: 65px;}th {    text-align: center;background-color: #9D1D27;    color: white;}</style>--><div style="margin-top:0.3em;width:100%;display:inline-block;border-radius:10px;overflow:hidden"><table class="easychat-bot-table"><tbody>';
        for(let row_index=0; row_index<table_list.length; row_index++){
            ...
            if (row_index==0){
                 ...
                 for(col_index=0; col_index<table_list[row_index].length; col_index++){
...
function append_datepicker(datepicker_list){
    ...
    {
        var datepicker_container = document.getElementById("easychat-datepicker-container");
...
function append_selected_date_into_user_query(element){
    var easychat_datepicker_list = document.getElementsByClassName("easychat-datepicker");
    var user_input = "";
...
function append_bot_range_slider(range_slider_list){
    ...
    {
        var easychat_range_slider_container = document.getElementById("easychat-range-slider-container");
...
function append_selected_value_into_user_query(element){
    var range_count = element.id.split("-")[3];
    document.getElementById("easychat-range-slider-value-"+range_count).innerHTML = element.value;
    var user_input = "";
    var easychat_range_slider_list = document.getElementsByClassName("easychat-range-slider");
    for(let i=0;i<easychat_range_slider_list.length;i++){
        var range_slider_element = easychat_range_slider_list[i];
...
function append_welcome_message(bot_id, bot_name){
    ...
    xhttp.onload = function(){
        if(this.status==200){
            ...
            if(carousel_img_url_list.length){add_banner(carousel_img_url_list,redirect_url_list)}
            if(carousel_img_url_list.length){init_gallery()}
...
function showSlides(n,el) {
  ...
  for(let i=0;i<c.length;i++){
     if(c[i].className == "mySlides fade easychat-slider-card" || c[i].className == "mySlides fade"){
        slides.push(c[i])
     }
  }
...
function init_gallery(){
    ...
    gallery_dots=[];
    try{
    var dotsContainer=document.getElementById("GallarydotsContainer"),i;
...
function FeedbackModal(el){
    var temp_value = document.getElementsByClassName("circle-value")
    document.getElementById("easychat-exit-app-feedback").style.display = "none";
    for(let i=0; i<temp_value.length;i++){temp_value[i].style.strokeWidth= "1.5em"}
    el.style.strokeWidth= "2em"
    contentvalue = parseInt(el.getAttribute("value"))
    if(contentvalue < 10){document.getElementById("value-0"+el.getAttribute("value")).style.strokeWidth= "2em"}
    else{document.getElementById("value-"+el.getAttribute("value")).style.strokeWidth= "2em"}
...
function handle_parent_message(event) {
    ...
    if("sender" in data && "agent_name" in data && data.sender != "client"){
        data.agent_name = "Agent: " + data.agent_name;
    }

    if("show_agent_message" in data) {
        hide_bot_typing_loader();
        if(data.chat_type == "chat_bubble"){
            append_system_text_response(data.message);
        } else {
            if(data.attachment != null) {
                let attachment_message = data.message;
                ...
                append_attachment_and_message_in_chat(attachment_message, data.attachment, data.attachment_file_name, time, "agent", data.agent_name,data.agent_profile_pic_source);
            } else {
                if(data.message != null && data.message.trim() != "") {
                    append_bot_text_response(data.message, data.agent_name,time,data.agent_profile_pic_source);
                }
            }
        }
    }
    else if("show_client_message" in data) {
        hide_bot_typing_loader();
        if(data.chat_type == "chat_bubble"){
            append_system_text_response(data.message);
        } else {
            if(data.attachment != null) {
                ...
                append_attachment_and_message_in_chat(attachment_message, data.attachment, data.attachment_file_name, time, "client");
            } else {
                if(data.message != null && data.message.trim() != "") {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                if(response["status"]==200){
                    ...
                    if(response["file_upload_fail"].length) {
                        ...
                        for(idx = 0; idx < file_upload_fail.length; idx ++) {
...
function append_client_attachment_and_message_in_chat(attachement_message, file_src, file_name, time) {
...
function append_agent_attachment_and_message_in_chat(attachement_message, file_src, file_name, time, name, agent_profile_pic_source="") {
    var extension = file_name.split(".")[1];
    var html = "";
    var attachment_message_html = "";
    var file_name_in_url = file_name.replace(/([\""|\''])/g, '\\$1');
...
function append_attachment_and_message_in_chat(attachement_message, file_src, file_name, time, sender, name, agent_profile_pic_source) {
    ...
    if(sender == "client") {
        append_client_attachment_and_message_in_chat(attachement_message, file_src, file_name, time)
    } else {
        append_agent_attachment_and_message_in_chat(attachement_message, file_src, file_name, time, name, agent_profile_pic_source);
    }
}

function append_attachment_send_message(element){
    var attachment_message = document.getElementById("attachment-message").value;
    ...
    for(let idx = 0; idx < uploaded_attached_files.length; idx ++) {
        ...
        append_attachment_and_message_in_chat(attachment_message, attached_file_src, attachment_file_name, time, "client");
        parent.postMessage(JSON.stringify({
            event_id: 'cobrowsing-client-chat-message',
            data: {
...
function update_agent_used_canned_response_reverse(keyword) {
    let url_parameters = new URLSearchParams(location.href);
    let session_id = url_parameters.get("session_id");
    ...
    xhttp.setRequestHeader('Content-Type', 'application/json');
    xhttp.setRequestHeader('X-AccessToken', ACCESS_TOKEN);
