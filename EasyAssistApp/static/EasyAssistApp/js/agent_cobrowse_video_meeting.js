...
async function start_client_audio_voip_record() {
    ...
    if (audio_recorder_on) {
        ...
        function save_data(blob) {
            ...
            $.ajax({
                url: "/easy-assist/save-agent-recorded-data/",
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
...
window.onload = function() {
    ...
    set_easyassist_current_session_local_storage_obj("is_video_pip_meeting_open", "true");

    window.addEventListener('beforeunload', function(event) {
        set_easyassist_current_session_local_storage_obj("is_video_pip_meeting_open", "false");
    });
...
function toggle_agent_voice(status) {
    is_audio_muted = !status;
    jitsi_meet_api.executeCommand('toggleAudio');
}

function toggle_agent_video(status) {
...
function handle_parent_message(event) {
    ...
    if(data.message == "toggle_voice") {
        toggle_agent_voice(data.status);
    } else if(data.message == "toggle_video") {
        toggle_agent_video(data.status);
