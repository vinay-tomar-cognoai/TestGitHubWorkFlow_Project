    ...
    try:
        ...
        if settings.DEBUG:
            cognoai_websocket.connect(
                "ws://127.0.0.1:8000/ws/cognoai-signal/" + str(websocket_token) + "/server/")
        else:
            cognoai_websocket.connect("wss://" + settings.EASYCHAT_DOMAIN +
                                      "/ws/cognoai-signal/" + str(websocket_token) + "/server/")
        ...
        logger.error("send_data_from_server_to_client: " + str(e) +
                     " at " + str(exc_tb.tb_lineno), extra={'AppName': 'EasyAssistSalesforce'})
