...
function load_agent_basic_analytics(start_date, end_date, title) {
	...
	let encrypted_data = custom_encrypt(json_params);
	encrypted_data = {
	...
	xhttp.open("POST", "/easy-assist-salesforce/agent/basic-analytics-outbound/?salesforce_token=" + window.SALESFORCE_TOKEN, true);
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			document.getElementById("div-total-sr-closed").innerHTML = 138;
			if (document.getElementById("div-total-sr-attended") != undefined){
				document.getElementById("div-total-sr-attended").innerHTML = 124;
			}
			if (document.getElementById("div-agent-nps") != undefined) {
				document.getElementById("div-agent-nps").innerHTML =78;
            }
            if (document.getElementById("div-total-sr-lead-captured") != undefined) {
				document.getElementById("div-total-sr-lead-captured").innerHTML = 138;
            }
            document.getElementById("div-total-sr-customer-denied").innerHTML = 3;
		}
	}
	xhttp.send(params);
}
...
function load_service_request_analytics(start_date, end_date, title, timeline) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/service-request-analytics-outbound/?salesforce_token=" + window.SALESFORCE_TOKEN, true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response["status"] == 200) {
				...
				let label_list = [];
				let total_sr_list = [45, 65, 30, 35, 50, 75, 60, 40];
				let total_sr_closed_list = [40, 62, 27, 34, 48, 74, 55, 38];
				let total_remaining_sr_list = [5, 3, 3, 1, 2, 1, 5, 2];
				let total_sr_attended_list = [42, 65, 28, 35, 48, 75, 57, 39];
				let total_sr_not_initiated_after_request_list = [3, 5, 6, 4, 8, 6, 9, 8];
				let total_sr_not_initiated_after_assigned_list = [2, 4, 5, 3, 6, 2, 8, 6];
				let total_sr_customer_denied_list=[5, 5, 3, 4, 3, 6, 5, 3]
				let total_sr_lead_captured_list=[45, 65, 30, 35, 50, 75, 60, 40]
				let min_messages = 20;
				let max_messages = 75;
				...
				for(let i = 0; i < response["service_request_analytics"].length; i++) {
                    ...
                    // total_sr_lead_captured = response["service_request_analytics"][i]["total_sr_lead_captured"];
                    // total_sr_customer_denied = response["service_request_analytics"][i]["total_sr_customer_denied"];
...
function load_agent_wise_request_analytics(start_date, end_date, title) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/agent-wise-request-analytics-outbound/?salesforce_token=" + window.SALESFORCE_TOKEN, true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
            ...
            response = JSON.parse(response);
			if (response.status == 200) {
				label_list = ["agent1", "agent2", "agent3", "agent4"];
				total_sr_list = [10, 15, 12, 17];
				total_sr_closed_list = [6, 11, 6, 13];
				let total_sr_not_closed_list = [2, 1, 2, 3];
				total_sr_attended_list = [8, 12, 8, 15];
				let total_sr_not_attended_list = [3, 2, 4, 1];
				total_sr_customer_denied_list=[5, 5, 3, 4];

				min_messages = 6;
				max_messages = 10;

				// for(let i = 0; i < response["agent_request_analytics_list"].length; i++) {
				// 	label_list.push(response["agent_request_analytics_list"][i]["agent"]);
				// 	total_sr = response["agent_request_analytics_list"][i]["total_sr"];
				// 	total_sr_closed = response["agent_request_analytics_list"][i]["total_sr_closed"];
                //     total_sr_attended = response["agent_request_analytics_list"][i]["total_sr_attended"];
                //     total_sr_customer_denied = response["agent_request_analytics_list"][i]["total_sr_customer_denied"];
				...
				Chart.helpers.each(Chart.instances, function(instance) {
					if (instance.chart.canvas.id == "agent-wise-request-analytics") {
						instance.destroy();
					}
				});

				// Area Chart Example
				var ctx = document.getElementById("agent-wise-request-analytics");
				var myLineChart = new Chart(ctx, {
					type: 'line',
					data: {
						labels: label_list,
...
function load_agent_wise_nps_analytics(start_date, end_date, title, page) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/agent-wise-nps-analytics-outbound/?salesforce_token=" + window.SALESFORCE_TOKEN, true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response.status == 200) {
				...
				let agent_nps_analytics_list = [{"nps":78, "agent": "agent1"}, {"nps":69, "agent": "agent2"}, {"nps":89, "agent": "agent3"}, {"nps":90, "agent": "agent4"}];
...
function load_page_wise_analytics(start_date, end_date, title, page) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/query-page-analytics-outbound/?salesforce_token=" + window.SALESFORCE_TOKEN, true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response.status == 200) {
				...
				let query_pages = [{"title":"Apply for education loan", "total": 50}, {"title":"Open current account", "total": 150}, {"title":"Open saving account", "total": 100}];
