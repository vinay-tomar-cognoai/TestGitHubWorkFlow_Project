...
function remove_expose_md5_hashed_element(){
        ...
        for(var index=0;index<form_button_elements.length;index++){
...
function save_html_element_value_on_focusin_event(){
	...
	if(this.type=="text" || this.type=="textarea"){
		...
		radio_elements = document.getElementsByName(this.name);
		for(var index=0;index<radio_elements.length;index++){
	...
	xhttp.onreadystatechange = function(){
		if(this.readyState==4 && this.status==200){
			...
			if(response.status==200 && response.highlight_text!=null & response.highlight_text!=undefined){
				...
				html_element_tag = response.highlight_text.html_element_tag;
				md5hash_str = response.highlight_text.md5hash_str;
...
function get_input_html_elements(){
	...
	for(var index=0;index<form_input_elements.length;index++){
		...
		md5_string = get_md5_string(form_input_elements[index]);
                ...
                md5_string_focused_element = get_md5_string(focused_element);
...
function connect_with_agent(client_name, client_mobile_number, agent_username=null){
	...
	html_input_elements = get_input_html_elements();
	html_select_elements = get_select_html_elements();
	html_textarea_elements = get_textarea_elements();
	focused_element = get_focused_element();
	md5_focused_element = get_md5_string(focused_element);

	end_date = new Date();
	total_time_spent = (end_date.getTime() - start_date.getTime())/1000;
...
function highlight_agent_selected_element(mouse_event_details){
	clientX = mouse_event_details["last_client_x"];
	clientY = mouse_event_details["last_client_y"];
	window.scrollTo(mouse_event_details["agent_window_x_offset"], mouse_event_details["agent_window_y_offset"]);
	agent_window_width = mouse_event_details["agent_window_width"];
	agent_window_height = mouse_event_details["agent_window_height"];
...
function sync_screen_to_agent(){
	...
	xhttp.onreadystatechange = function(){
		if(this.readyState==4 && this.status==200){
			response = JSON.parse(this.responseText);
			if(response.status==200){
				html_elements = response.html_elements;
...
function string_exists_in_list(str_list, str){
    for(var index=0;index<str_list.length;index++){
...
function remove_floating_disconnect_button(){
    client_disconnect = document.getElementById("img-client-disconnected");
