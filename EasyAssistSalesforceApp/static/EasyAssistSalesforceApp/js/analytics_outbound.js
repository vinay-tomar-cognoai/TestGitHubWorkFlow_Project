	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response["status"] == 200) {
                ...
                total_sr_lead_captured_list = [];
                total_sr_customer_denied_list = [];
					...
					'<table class="table table-bordered" id="agent-session-analytics-data-table" width="100%" cellspacing="0">',
						'<thead>',
							'<tr>',
								...
								'<th>Cobrowsing Request denied by Customer</th>',
							'</tr>',
						'</thead>',
						'<tbody>',
							html_table,
...
function get_visited_page_title_list() {
	...
	const params = JSON.stringify(encrypted_data);
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/get-visited-page-title-list-outbound/?salesforce_token=" + window.SALESFORCE_TOKEN, true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
