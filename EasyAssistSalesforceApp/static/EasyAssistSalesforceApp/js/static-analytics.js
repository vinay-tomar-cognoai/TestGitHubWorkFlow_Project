...
function load_agent_basic_analytics(start_date, end_date, title) {
	...
	xhttp.open("POST", "/easy-assist-salesforce/agent/basic-analytics/", true);
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			document.getElementById("div-total-sr").innerHTML = 120;
			document.getElementById("div-total-sr-closed").innerHTML = 110;
			if (document.getElementById("div-total-sr-attended") != undefined){
				document.getElementById("div-total-sr-attended").innerHTML = 115;
			}
			if (document.getElementById("div-total-sr-not-closed") != undefined){
				document.getElementById("div-total-sr-not-closed").innerHTML = 10;
			}
			if (document.getElementById("div-total-sr-not-initiated-after-request") != undefined) {
				document.getElementById("div-total-sr-not-initiated-after-request").innerHTML = 7;
			}
			if (document.getElementById("div-total-sr-not-initiated-after-assigned") != undefined){
				document.getElementById("div-total-sr-not-initiated-after-assigned").innerHTML = 7;
			}
			if (document.getElementById("div-agent-nps") != undefined) {
				document.getElementById("div-agent-nps").innerHTML = 89;
			}
		}
	}
	xhttp.send(params);
...
function load_service_request_analytics(start_date, end_date, title, timeline) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/service-request-analytics/", true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response["status"] == 200) {
				...
				label_list = [];
				total_sr_list = [45, 65, 30, 35, 50, 75, 60, 40];
				total_sr_closed_list = [40, 62, 27, 34, 48, 74, 55, 38];
				total_remaining_sr_list = [5, 3, 3, 1, 2, 1, 5, 2];
				total_sr_attended_list = [42, 65, 28, 35, 48, 75, 57, 39];
				total_sr_not_initiated_after_request_list = [3, 5, 6, 4, 8, 6, 9, 8];
				total_sr_not_initiated_after_assigned_list = [2, 4, 5, 3, 6, 2, 8, 6];
...
function load_agent_wise_request_analytics(start_date, end_date, title) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/agent-wise-request-analytics/", true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response.status == 200) {
				...
				total_sr_not_closed_list = [2, 1, 2, 3];
				total_sr_attended_list = [8, 12, 8, 15];
				total_sr_not_attended_list = [3, 2, 4, 1];
				...
				var myLineChart = new Chart(ctx, {
					...
					data: {
						...
						datasets: [{
							...
							label: "Cobrowsing Request Unattended",
							lineTension: 0.3,
							backgroundColor: "rgb(255, 102, 0, 0.05)",
							borderColor: "rgb(255, 102, 0, 1)",
							pointRadius: 3,
							...
							pointHitRadius: 10,
							pointBorderWidth: 2,
							data: total_sr_not_attended_list,
						}],
					},
					options: {
						maintainAspectRatio: false,
...
function load_agent_wise_nps_analytics(start_date, end_date, title, page) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/agent-wise-nps-analytics/", true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response.status == 200) {
				...
				agent_nps_analytics_list = [{"nps":79, "agent": "agent1"}, {"nps":69, "agent": "agent2"}, {"nps":89, "agent": "agent3"}, {"nps":90, "agent": "agent4"}];
...
function load_page_wise_analytics(start_date, end_date, title, page) {
	...
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/query-page-analytics/", true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if (response.status == 200) {
				...
				query_pages = [{"title":"Apply for education loan", "total": 30}, {"title":"Open current account", "total": 150}, {"title":"Open saving account", "total": 100}];
...
function get_visited_page_title_list() {
	query_pages = ["Apply for education loan", "Open current account", "Open saving account"];
	...
	const params = JSON.stringify(encrypted_data);
	var xhttp = new XMLHttpRequest();
	xhttp.open("POST", "/easy-assist-salesforce/agent/get-visited-page-title-list/", true);
	xhttp.setRequestHeader('Content-Type', 'application/json');
	xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
