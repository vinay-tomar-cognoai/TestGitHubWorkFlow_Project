...
function init() {
    ...
    chat_report_status_timer = setInterval(function() {
...
function save_vc_meeting_duration () {
    ...
    var xhttp = new XMLHttpRequest();
    xhttp.open("POST", "/livechat/save-meeting-duration/", true);
...
function redirect_end_meeting_page() {
    window.location = "/livechat/agent-vc-meeting-end/?meeting_id=" + meeting_id + `&language=${selected_language}`;
}
...
async function start_recording() {
    console.log(check_meeting_recording_enabled());
    if(!check_meeting_recording_enabled()) {
        return;
    }
    var is_mobile = detect_mobile();
    if(is_mobile == false){
        ...
        if (screen_recorder_on) {
            ...
            function save_data(blob) {
                ...
                $.ajax({
                    url: "/livechat/save-screen-recorded-data/",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
...
function end_meeting(client_id) {
    if (client_websocket_open && client_websocket != null) {
        client_websocket.send(JSON.stringify({
            "message": {
                "type": "11",
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response.status == 200) {
                ...
                if (meeting_doc_list.length > 0) {
                    for(var idx = 0; idx < meeting_doc_list.length; idx++) {
                        div_inner_html += [
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            ...
            if (speed < 2048) {
                ...
                clearInterval(chat_report_status_timer)
            }
        }
    }
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["status"] == 200) {
                ...
                for (var i = 0; i < message_history.length; i ++) {
                    ...
                    var message_bubble_align_class = "";
...
function check_chat_report_status() {
    ...
    var xhttp = new XMLHttpRequest();
    xhttp.open("POST", "/livechat/check-chat-report-status/", true);
    xhttp.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    xhttp.setRequestHeader('X-CSRFToken', get_csrfmiddlewaretoken());
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
