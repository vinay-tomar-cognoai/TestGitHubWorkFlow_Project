...
$(window).ready(function () {
    ...
    setInterval(() => {
        initialize_vc_history_table(state.page, true);
    ...
    $("#refresh_voip_history_btn").on("click", () => {
        initialize_vc_history_table(state.page);
    });
...
export async function initialize_vc_history_table(
    page,
    auto_refresh = false
) {
    if (!auto_refresh) {
        toggle_loader("show");
        $("#livechat_vc_history_no_data").css("display", "none");
        $("#livechat_vc_history_table_div").css("display", "none");
    }

    state.page = page;

    const response = await get_vc_history(page);
    ...
    if (response.total_audits > 0) {
        ...
        document.getElementById("livechat_vc_history_no_data").style.display = "none";

        $("#livechat_vc_history_table_div").html(table_html);
        $("#livechat_vc_history_table_div").css("display", "block");
        ...
        document.getElementById("livechat_vc_history_table_div").style.display = "none";
        
        document.getElementById("livechat_vc_history_no_data").style.display = "block";
    }
}
...
export function get_vc_history(page) {
...
function add_pagination_events() {
    $(".audit-trail-pagination").on("click", (event) => {
        initialize_vc_history_table(event.target.dataset.page);
    });
}
...
function close_vc_history_export_modal() {
    $('#modal-export-vc-history').modal('toggle');
}


export function export_vc_filter() {
