...
def get_call_type(agent_obj, LiveChatConfig):
    try:
        bot_obj = agent_obj.bots.all()[0]

        config_obj = LiveChatConfig.objects.get(bot=bot_obj)

        return config_obj.call_type

    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("Error get_call_type %s at %s",
                     str(e), str(exc_tb.tb_lineno), extra={'AppName': 'LiveChat'})

    return 'none'


def audio_merger(audio_paths, output_filename):
    logger.info("Inside merger...", extra={"AppName": "LiveChat"})
    try:

        merged_audio = ffmpeg.input(
            audio_paths[0][1:]).audio

        for index in range(1, len(audio_paths)):
            added_audio = ffmpeg.input(
                audio_paths[index][1:]).audio
            merged_audio = ffmpeg.filter([merged_audio, added_audio], 'amix')

        merged_file_path = "secured_files/LiveChatApp/merged_files/"
        if not os.path.exists(merged_file_path):
            os.makedirs(merged_file_path)

        out, err = (
            ffmpeg
            .output(merged_audio, merged_file_path + output_filename)
            .run(overwrite_output=True)
        )
        logger.info("Merging done...", extra={"AppName": "LiveChat"})
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        logger.error("In audio_merger: %s at %s", str(e), str(exc_tb.tb_lineno), extra={
            'AppName': 'LiveChat'})
        return "None"
