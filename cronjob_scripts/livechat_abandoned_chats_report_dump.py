...
def cronjob():
    try:
        ...
        try:
            ...
            for user in LiveChatUser.objects.filter(is_deleted=False, status__in=["1", "2"]):
                ...
                if not os.path.exists(settings.MEDIA_ROOT + 'livechat-abandoned-chats-report/' + str(user.user.username)):
                    os.makedirs(
                        settings.MEDIA_ROOT + 'livechat-abandoned-chats-report/' + str(user.user.username))
                ...
                for last_x_day in range(1, total_x_days + 2):
                    ...
                    if last_x_day == total_x_days + 1:
                        ...
                        if os.path.exists(settings.MEDIA_ROOT + "livechat-abandoned-chats-report/" + str(user.user.username) + "/abandoned_chats_report_" + str(day_x.date()) + ".xls"):
                            cmd = "rm " + settings.MEDIA_ROOT + "livechat-abandoned-chats-report/" + \
                    ...
                    if path.exists(settings.MEDIA_ROOT + "livechat-abandoned-chats-report/" + str(user.user.username) + "/abandoned_chats_report_" + str(last_date.date()) + ".xls"):
                        file_already_exists = True

                    if not file_already_exists:
                        message_history_yesterday = LiveChatCustomer.objects.filter(
                            last_appearance_date__date=last_date.date(), agent_id=None, abruptly_closed=True, category__in=category_list, bot__in=user.bots.all(
                            ))
                        ...
                        filename = "livechat-abandoned-chats-report/" + \
                            str(user.user.username) + "/abandoned_chats_report_" + \
        ...
        for export_date_request in LiveChatDataExportRequest.objects.filter(is_completed=False, report_type="9"):
            try:
                ...
                while temp_date <= end_date:
                    file_path = settings.MEDIA_ROOT + "livechat-abandoned-chats-report/" + \
                        ...
                        "/abandoned_chats_report_" + str(temp_date) + ".xls"

                    if path.exists(file_path):
                        temp_date = temp_date + timedelta(1)
                        continue
                    message_history_yesterday = LiveChatCustomer.objects.filter(
                        last_appearance_date__date=temp_date, agent_id=None, abruptly_closed=True, bot__in=user.bots.all(), category__in=category_list)
                ...
                export_zip_file_path = "livechat-abandoned-chats-report/" + \
                    str(user.user.username) + "/AbandonedChatsReportCustom-" + \
                ...
                body = """
                    ...
                    <div style="padding:1em;border:0.1em black solid;" class="container">
                        ...
                        <p>
                            We have received a request to provide you with the LiveChat Abandoned Chats report from {} to {}. Please click on the link below to download the file.
                        </p>
                        <a href="{}/{}">click here</a>
                        <p>&nbsp;</p>
                        """
                ...
                for email_id in email_list:
                    ...
                    email_subject = "LiveChat Abandoned Chats History For " + \
                        str(username)
                ...
                exc_type, exc_obj, exc_tb = sys.exc_info()
                logger.error("Abandoned Chats Datadump! %s %s", str(e), str(exc_tb.tb_lineno), extra={
                             'AppName': 'LiveChat'})
                message_history_datadump_log.write(
                    str(today) + ": failed: " + str(e))
