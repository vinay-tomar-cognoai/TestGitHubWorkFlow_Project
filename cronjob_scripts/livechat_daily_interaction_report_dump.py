...
def cronjob():
    try:
            ...
            for user in LiveChatUser.objects.filter(is_deleted=False, status__in=["1", "2"]):
                ...
                for last_x_day in range(1, total_x_days + 2):
                    ...
                    if last_x_day == total_x_days + 1:
                        ...
                        if os.path.exists(settings.MEDIA_ROOT + 'livechat-daily-interaction-report/' + str(user.user.username) + '/daily_interaction_report_' + str(day_x.date()) + '.xls'):
                            cmd = "rm " + settings.MEDIA_ROOT + "livechat-daily-interaction-report/" + \
                                str(user.user.username) + \
                                "/daily_interaction_report_" + \
                                str(day_x.date()) + ".xls"
                            os.system(cmd)
                        break
                    ...
                    if path.exists(settings.MEDIA_ROOT + "livechat-daily-interaction-report/" + str(user.user.username) + "/daily_interaction_report_" + str(last_date.date()) + ".xls"):
                    ...
                    if not file_already_exists:
                        ...
                        interaction_object_list = LiveChatCustomer.objects.filter(
                            agent_id__in=agent_list, request_raised_date=last_date.date())
                        ...
                        filename = 'livechat-daily-interaction-report/' + \
        ...
        for export_date_request in LiveChatDataExportRequest.objects.filter(is_completed=False, report_type="3"):
            try:
                ...
                while temp_date <= end_date:
                    ...
                    file_path = "livechat-daily-interaction-report/" + \
                        str(user.user.username) + \
                        "/daily_interaction_report_" + str(temp_date) + ".xls"
                    test_wb.save(settings.MEDIA_ROOT + file_path)

                    temp_date = temp_date + timedelta(1)

                export_zip_file_path = "livechat-daily-interaction-report/" + \
                    str(user.user.username) + "/DailyInteractionReportCustom-" + \
                ...
                body = """
                    ...
                    <div style="padding:1em;border:0.1em black solid;" class="container">
                        ...
                        <p>
                            We have received a request to provide you with the LiveChat Daily Interaction report from {} to {}. Please click on the link below to download the file.
                        </p>
                        <a href="{}/{}">click here</a>
                        <p>&nbsp;</p>
                        """
                ...
                for email_id in email_list:
                    ...
                    email_subject = "LiveChat Daily Interaction For " + \
                        str(username)
