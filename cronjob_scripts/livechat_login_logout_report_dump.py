...
def cronjob():
    try:
            ...
            for user in LiveChatUser.objects.filter(is_deleted=False, status__in=["1", "2"]):
                ...
                for last_x_day in range(1, total_x_days + 2):
                    ...
                    if last_x_day == total_x_days + 1:
                        ...
                        if os.path.exists(settings.MEDIA_ROOT + "livechat-login-logout-report/" + str(user.user.username) + "/login_logout_report_" + str(day_x.date()) + ".xls"):
                            cmd = "rm " + settings.MEDIA_ROOT + "livechat-login-logout-report/" + \
                                str(user.user.username) + \
                                "/login_logout_report_" + \
                                str(day_x.date()) + ".xls"
                            os.system(cmd)
                        break
                    ...
                    if path.exists(settings.MEDIA_ROOT + "livechat-login-logout-report/" + str(user.user.username) + "/login_logout_report_" + str(last_date.date()) + ".xls"):
                        file_already_exists = True

                    if not file_already_exists:

                        message_history_yesterday = LiveChatSessionManagement.objects.filter(
                            session_starts_at__date=last_date.date(), user__in=agent_list).order_by('-session_starts_at')
                        ...
                        for message in message_history_yesterday:
                            ...
                            sheet1.write(
                                row, 1, str(message.session_starts_at.date().strftime("%d-%m-%Y")))
                            sheet1.write(
                                row, 2, message.get_session_duration())
                            sheet1.write(row, 3, message.get_name())
                            sheet1.write(
                                row, 4, str(message.session_starts_at.time().strftime("%H:%M:%S")))
                            sheet1.write(
                                row, 5, str(message.session_ends_at.time().strftime("%H:%M:%S")))
                            sheet1.write(
                                row, 6, str(message.get_total_offline_time()))
                            sheet1.write(
                                row, 7, str(message.get_total_online_time()))
                            sheet1.write(
                                row, 8, str(message.get_session_stop_interaction_duration()))
                            row += 1

                        filename = "livechat-login-logout-report/" + \
        ...
        for export_date_request in LiveChatDataExportRequest.objects.filter(is_completed=False, report_type="5"):
            try:
                ...
                export_zip_file_path = "livechat-login-logout-report/" + \
                    str(user.user.username) + "/LoginLogoutReportCustom-" + \
                ...
                body = """
                    ...
                    <div style="padding:1em;border:0.1em black solid;" class="container">
                        ...
                        <p>
                            We have received a request to provide you with the LiveChat Login-Logout report from {} to {}. Please click on the link below to download the file.
                        </p>
                        <a href="{}/{}">click here</a>
                        <p>&nbsp;</p>
                        """
                ...
                for email_id in email_list:
                    ...
                    email_subject = "LiveChat Login Logout History For " + \
                        str(username)
                ...
                exc_type, exc_obj, exc_tb = sys.exc_info()
                logger.error("Login Logout Datadump! %s %s", str(e), str(exc_tb.tb_lineno), extra={
                             'AppName': 'LiveChat'})
                message_history_datadump_log.write(
                    str(today) + ": failed: " + str(e))
