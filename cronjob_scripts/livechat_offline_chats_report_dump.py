        ...
        try:
            ...
            for user in LiveChatUser.objects.filter(is_deleted=False, status__in=["1", "2"]):
                ...
                if not os.path.exists(settings.MEDIA_ROOT + 'livechat-offline-chats-report/' + str(user.user.username)):
                    os.makedirs(
                        settings.MEDIA_ROOT + 'livechat-offline-chats-report/' + str(user.user.username))
                for last_x_day in range(1, total_x_days + 2):

                    if last_x_day == total_x_days + 1:
                        day_x = today - timedelta(last_x_day)
                        if os.path.exists(settings.MEDIA_ROOT + "livechat-offline-chats-report/" + str(user.user.username) + "/offline_chats_report_" + str(day_x.date()) + ".xls"):
                            cmd = "rm " + settings.MEDIA_ROOT + "livechat-offline-chats-report/" + \
                    ...
                    if path.exists(settings.MEDIA_ROOT + "livechat-offline-chats-report/" + str(user.user.username) + "/offline_chats_report_" + str(last_date.date()) + ".xls"):
                        file_already_exists = True

                    if not file_already_exists:

                        message_history_yesterday = LiveChatCustomer.objects.filter(
                            last_appearance_date__date=last_date.date(), is_system_denied=True, bot__in=user.bots.all(
                        ...
                        filename = "livechat-offline-chats-report/" + \
                            str(user.user.username) + "/offline_chats_report_" + \
                            str(last_date.date()) + ".xls"
                        test_wb.save(settings.MEDIA_ROOT + filename)
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            logger.error("Offline Chats Datadump! %s %s", str(e), str(exc_tb.tb_lineno), extra={
                         'AppName': 'LiveChat'})
            message_history_datadump_log.write(
                str(today) + ": failed: " + str(e))

        for export_date_request in LiveChatDataExportRequest.objects.filter(is_completed=False, report_type="8"):
            try:
                ...
                while temp_date <= end_date:
                    file_path = settings.MEDIA_ROOT + "livechat-offline-chats-report/" + \
                        ...
                        "/offline_chats_report_" + str(temp_date) + ".xls"

                    if path.exists(file_path):
                        temp_date = temp_date + timedelta(1)
                        continue
                    message_history_yesterday = LiveChatCustomer.objects.filter(
                        last_appearance_date__date=temp_date, is_system_denied=True, bot__in=user.bots.all(), category__in=category_list)
                ...
                export_zip_file_path = "livechat-offline-chats-report/" + \
                    str(user.user.username) + "/OfflineChatsReportCustom-" + \
                ...
                body = """
                    ...
                    <div style="padding:1em;border:0.1em black solid;" class="container">
                        ...
                        <p>
                            We have received a request to provide you with the LiveChat Offline Chats Report from {} to {}. Please click on the link below to download the file.
                        </p>
                        <a href="{}/{}">click here</a>
                        <p>&nbsp;</p>
                        """
