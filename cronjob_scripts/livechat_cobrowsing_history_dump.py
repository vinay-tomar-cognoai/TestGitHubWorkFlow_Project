...
def cronjob():
    try:
            ...
            for user in LiveChatUser.objects.filter(is_deleted=False, status__in=["1", "2"]):
                ...
                for last_x_day in range(1, total_x_days + 2):
                    ...
                    if last_x_day == total_x_days + 1:
                        ...
                        if os.path.exists(settings.MEDIA_ROOT + 'livechat-cobrowsing-history/' + str(user.user.username) + '/cobrowsing_history_' + str(day_x.date()) + '.xls'):
                            cmd = 'rm ' + settings.MEDIA_ROOT + 'livechat-cobrowsing-history/' + \
                    ...
                    if path.exists(settings.MEDIA_ROOT + "livechat-cobrowsing-history/" + str(user.user.username) + "/cobrowsing_history_" + str(last_date.date()) + ".xls"):
                    ...
                    if not file_already_exists:
                        ...
                        cobrowsing_history_objs = LiveChatCobrowsingData.objects.filter(request_datetime__date=last_date.date(), agent__in=agent_list, is_completed=True, is_interrupted=False).order_by("-request_datetime")
                        ...
                        row = 1
                        for cobrowsing_data in cobrowsing_history_objs:

                            sheet1.write(row, 0, cobrowsing_data.customer.get_username())
                            sheet1.write(row, 1, cobrowsing_data.agent.user.username)

                            sheet1.write(row, 2, cobrowsing_data.start_datetime.astimezone(tz).strftime('%d-%b-%Y, %I:%M %p'))
                            sheet1.write(row, 3, cobrowsing_data.end_datetime.astimezone(tz).strftime('%d-%b-%Y, %I:%M %p'))
                        ...
                        filename = 'livechat-cobrowsing-history/' + \
                            str(user.user.username) + '/cobrowsing_history_' + \
        ...
        for export_date_request in LiveChatDataExportRequest.objects.filter(is_completed=False, report_type="12"):
            try:
                ...
                while temp_date <= end_date:
                    ...
                    file_path = settings.MEDIA_ROOT + "livechat-cobrowsing-history/" + \
                        str(user.user.username) + "/cobrowsing_history_" + \
                        str(temp_date) + ".xls"
                    ...
                    cobrowsing_history_objs = LiveChatCobrowsingData.objects.filter(request_datetime__date=temp_date, agent__in=agent_list, is_completed=True, is_interrupted=False).order_by("-request_datetime")
                ...
                export_zip_file_path = "livechat-cobrowsing-history/" + \
                    str(user.user.username) + "/CobrowsingHistoryCustom-" + \
                ...
                body = """
                    ...
                    <div style="padding:1em;border:0.1em black solid;" class="container">
                        ...
                        <p>
                            We have received a request to provide you with the LiveChat Cobrowsing History from {} to {}. Please click on the link below to download the file.
                        </p>
                        <a href="{}/{}">click here</a>
                        <p>&nbsp;</p>
                        """
