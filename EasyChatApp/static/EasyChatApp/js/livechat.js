...
var chat_escalation_report_agent_message = false;
...
var is_transcript_enabled = false;
var is_transcript_sent = false;
var is_csat_transcript_enabled = false;
let is_queue_timer_set = false;
let livechat_queue_timer = null;
let is_weak_connection = false;
...
var CREATE_CUSTOMER = "/livechat/create-customer/";
var ASSIGN_AGENT = "/livechat/assign-agent/";
var MARK_CHAT_ABANDONED = "/livechat/mark-chat-abandoned/";
var SAVE_CUSTOMER_CHAT = "/livechat/save-customer-chat/";
...
$(document).ready(() => {
    document.addEventListener('visibilitychange', check_tab_focused);
});
...
(function (exports) {
    ...
    exports.assigned_agent_username = assigned_agent_username;
    ...
    function append_livechat_response(is_text_to_speech_required, speech_response) {
        ...
        if (show_livechat_form_or_no && customer_info_needed) {
    ...
    function clear_livechat_data(end_message, agent_inactivity_text_message="") {
        if (is_chat_socket_open == true && chat_socket != null) {
            if (left_msg_append == false) {
                if (livechat_session_id != "") {
                    ...
                    if(is_weak_connection)
                    {
                        livechat_notification = WEAK_INTERNET_OR_MINIMIZE_TEXT;
                    }
                    
                    if (is_chat_escalation_report_notified) {
                        end_message = chat_escalation_report_agent_message;
                    }
                    
                    save_customer_chat(
                        agent_inactivity_text_message ? agent_inactivity_text_message : end_message,
                        livechat_session_id,
                        "System",
                        "",
                        "",
                    ...
                    var sentence = JSON.stringify({
                        message: JSON.stringify({
                            text_message: agent_inactivity_text_message ? agent_inactivity_text_message : end_message,
                            type: "text",
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                ...
                if (response["status"] == 200) {
                    ...
                    is_transcript_sent = response.is_transcript_request;
                    
                    if(IS_TRANSCRIPT_ENABLED && !is_transcript_sent){
                        show_transcript_option();
                        show_csat_transcript_option();
                    } else if (IS_TRANSCRIPT_ENABLED && is_transcript_sent) {
                        hide_transcript_option();
                        hide_csat_transcript_option();
                    }
                    ...
                    if (response_objs.length == 0 && false) {
                        ...
                        unset_livechat_cookies_for_iOS();
                        if (livechat_session_id != "") {
                            mark_chat_unavailable_or_abandoned(livechat_session_id, false);
                            chat_socket.close();
                        }
                        ...
                        for (var i = 0; i < response_objs.length; i++) {
                            if(response_objs[i].is_copied_from_easychat) {
                                continue;
                            }
                            if (response_objs[i].sender == "Customer") {
                                ...
                                if ((response_objs[i].is_video_call_message || response_objs[i].is_voice_call_message || response_objs[i].is_cobrowsing_message || response_objs[i].is_transcript_message || response_objs[i].is_file_not_support_message)) {
                                    if (response_objs[i].message_for == 'customer') {
                                        append_system_text_response(response_objs[i].message);
                                        if (response_objs[i].is_transcript_message) {
                                            $(".option-for-receiving-transcript-div").css("display", "none");
                                        }
                                    }
                                ...
                                } else if(!response_objs[i].message_for){
                                    var text_message = response_objs[i].message;
                                    if(response_objs[i].translated_text != "")
                                        text_message = response_objs[i].translated_text;
                                    append_system_text_response(text_message);      
                                }
                            } else if(response_objs[i].sender == 'Bot') {
                                let text_message = response_objs[i].message;
                                if(response_objs[i].translated_text != "")
                                    text_message = response_objs[i].translated_text;
                                append_bot_text_response(text_message, undefined, undefined, undefined);
                            }
                        }
                        ...
                        get_livechat_special_character_toggle();

                    let is_queue_time_exceed = response.is_queue_time_exceed;

                    if (is_queue_time_exceed) {
                    ...
                    else if(is_queue_time_exceed != undefined) {
                        hide_end_chat_button_based_on_theme();
                        is_queue_timer_set = true;
                        append_queue_timer();
                        queue_timer_fun(response.queue_time, true);
                        assign_agent(true);
    ...
    function close_livechat(end_message, agent_inactivity_text_message="") {
        is_queue_timer_set = false;
        ...
        if (cobrowsing_status == 'ongoing' && !is_chat_escalation_report_notified) {
        ...
        unset_livechat_cookies();
        unset_livechat_cookies_for_iOS();
        clear_userData(end_message, agent_inactivity_text_message);
        save_time_spent();
        if (chat_socket)
            chat_socket.close();
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response.status_code == "200") {
                let customer_modal = document.getElementById("customer_info_form_modal");
                if(customer_modal){
                    customer_modal.remove();
                }
                ...
                try {
                    ...
                    if (channel_name && channel_name.toLowerCase().trim() == "android") {
                        android.setLiveChatSessionId(livechat_session_id);
                    }
                    if (channel_name && channel_name.toLowerCase().trim() == "ios") {

                        webkit.messageHandlers.setLiveChatSessionID.postMessage({"livechat_session_id": livechat_session_id});
                    }
                } catch (err) {
                    console.log(err);
                }
                ...
                assign_agent(false , true);
                check_agent_assign_timer = setInterval(assign_agent, 5000);
                let transcript_checkbox = document.getElementById("transcript-checkbox-hide");
                if (transcript_checkbox) { 
                    transcript_checkbox.style.display = "block";
                }
                ...
                } else if (assigned_agent.trim() == "non_working_hour")
                {
                    livechat_notification = response["message"];

                } else if (assigned_agent == "customer_blocked") {
                    livechat_notification = response["message"];
                }
...
function assign_agent(is_refreshed = false , is_initial_call = false) {
    if (!is_refreshed && (is_livechat == false || is_chat_socket_open == false || chat_socket == null)) {
        return;
    }
        ...
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (assigned_agent == "scheduler_queue") {
                ...
                if (is_chat_socket_open == true && need_to_add_wel_msg == true) {
                    var message = response.joined_chat_text;
                    if (is_iOS()){
                        let warning_message = response.warning_text_for_ios;
                        append_system_text_response(warning_message);
                    }
                    append_system_text_response(message);
                    need_to_add_wel_msg = false;
                    setTimeout(scroll_to_bottom, 300);
                    if(IS_TRANSCRIPT_ENABLED && is_transcript_sent == false){
                    show_transcript_option();
                    }
...
function check_and_close_queue_timer() {
    if (is_queue_timer_set) {
        clearInterval(livechat_queue_timer);
        is_queue_timer_set = false;
    }
...
function unset_livechat_cookies_for_iOS() {

    try {

        webkit.messageHandlers.setLiveChatSessionID.postMessage({"livechat_session_id": ""})
    } catch(err){}
}
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response["status_code"] == "200") {
                ...
                try {
                    if (channel_name && channel_name.toLowerCase().trim() == "android") {
                        android.setLiveChatSessionId("");
                    }
                    if (channel_name && channel_name.toLowerCase().trim() == "ios") {
                        webkit.messageHandlers.setLiveChatSessionId.postMessage("");
                    }
                } catch (err) {
                    console.log(err);
                }
...
function save_customer_chat(
    ...
    if (sentence == LIVECHAT_VOICECALL_START || sentence == LIVECHAT_VOICECALL_END || sentence == LIVECHAT_VOICECALL_REQUEST) {
        json_string['voice_call_notification'] = true
    }

    if (sentence == LIVECHAT_VC_START || sentence == LIVECHAT_VC_END || sentence == LIVECHAT_VC_REQUEST) {
        json_string['video_call_notification'] = true
    }

    if (sentence == CB_START || sentence == CB_END) {
        json_string['cobrowsing_notification'] = true
    }

    if (sentence == LIVECHAT_TRANSCRIPT_SEND_TEXT) {
        json_string['transcript_notification'] = true
    }
    
    if (sentence == LIVECHAT_EXCEED_FILE_SIZE_TEXT || sentence == LIVECHAT_VALID_FILE_TEXT) {
        json_string['file_not_support_notification'] = true
    }
...
function append_info_form() {
    ...
    var country_list_text = '', 
    country_list = $('#customer_info_form_div .country-list li'), 
    duplicate_country_remove = [];

    $(country_list).each(function () {
    var text = $(this).text();
    if (country_list_text.indexOf('|'+ text + '|') == -1){
        country_list_text += '|'+ text + '|'; 
    }
    else{
        duplicate_country_remove.push($(this));
    }
    })

    $(duplicate_country_remove).each(function () { $(this).remove(); });
...
function get_customer_info_form_html() {
    ...
    html +=
        '<input type="text" id="easychat-customer-phone" class = "easychat-customer-input"  autocomplete="off" oninput="paste_only_numbers(this)" style="direction:'+ pointer_direction +';">';
...
function submit_customer_info() {
    ...
    if (!is_valid) {
        ...
        if ($("#easychat-customer-phone").intlTelInput("getSelectedCountryData").dialCode == "91") {
            if (error_text == "" && !validate_phone_number(phone)) {
...
function queue_timer_fun(queue_temp_timer, is_refreshed = false) {
...
function show_transcript_option() {
   
    var html =
        '<div class="option-for-receiving-transcript-div">\
        <p class="option-for-receiving-transcript-para">\
            '+ LIVECHAT_CONVERSATION_MAIL_TEXT + ' \
            <a href="#" onclick="append_system_text_response(LIVECHAT_TRANSCRIPT_SEND_TEXT);check_for_send_transcript();hide_transcript_option()" >' + LIVECHAT_CLICK_HERE_TEXT + '</a> \
        </p>\
    </div>'
    chat_container.innerHTML += html;
}

function check_for_send_transcript() {
    
    let json_string = {
        session_id: livechat_session_id,
        is_agent_request: false,
        send_system_mail_message : true,
        system_message : LIVECHAT_TRANSCRIPT_SEND_TEXT, 
    };
    ...
    var xhttp = new XMLHttpRequest();
    var params = "json_string=" + json_string;
    xhttp.open("POST", '/livechat/enable-livechat-transcript/', false);
    xhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            response = this.response;
            ...
            if (response.status == 200) {
                let message = "The transcript will be sent over mail";
                save_customer_chat(message, livechat_session_id, "System", "", "", "", "");
                ...
                let sentence = JSON.stringify({
                    message: JSON.stringify({
                        ...
                        channel: window.channel_name_iframe,
                        path: "",
                        event_type: "IS_CUSTOMER_TRANSCRIPT_REQUEST",
                        is_transcript_system_message: true,
                    }),
                    sender: "System",
                });
                ...
                sentence = encrypt_variable(sentence);
                chat_socket.send(sentence);
                send_notification_to_agent(message,"","","IS_CUSTOMER_TRANSCRIPT_REQUEST");
                is_transcript_sent = true;
                hide_transcript_option();
...
function get_livechat_special_character_toggle() {
    ...
    xhttp.open("POST", '/livechat/get-livechat-special-character-toggle/', false);
    xhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response.status == 200) {
                window.IS_LIVECHAT_SPECIAL_CHARACTER_ALLOW_IN_FILE_NAME = response.is_special_character_allowed_in_file_name;
                window.IS_LIVECHAT_SPECIAL_CHARACTER_ALLOW_IN_CHAT = response.is_special_character_allowed_in_chat;
            } 
        }
    };
    xhttp.send(params);
...
function send_transcript(is_feedback_transcript_request=false) {
    let json_string = {
        session_id: livechat_session_id,
        is_agent_request: false,
        is_feedback_transcript_request: is_feedback_transcript_request,
    ...
    var xhttp = new XMLHttpRequest();
    var params = "json_string=" + json_string;
    xhttp.open("POST", '/livechat/livechat-transcript/', false);
    xhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            response = this.response;
            ...
            if (response.status == 200) {
                console.log('Success');
                if (is_feedback_transcript_request){
                    let is_link_disable = document.getElementById(`disable-click-link-${livechat_session_id}`);
                    if (is_link_disable) {
                        is_link_disable.classList.add("disable-link");
                        document.getElementById(`click-hear-svg-${livechat_session_id}`).innerHTML = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <rect width="24" height="24" rx="12" fill="#D1FAE5"/>
                        <path d="M20.2495 6.75049L9.74951 17.25L4.49951 12.0005" stroke="#059669" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>`
                    }
                }
            } else {
...
function hide_transcript_option() {
    $(".option-for-receiving-transcript-div").css("display","none")

}

function hide_csat_transcript_option() {
    document.getElementById("transcript-checkbox-hide").style.display = "none";

}

function show_csat_transcript_option() {
    document.getElementById("transcript-checkbox-hide").style.display = "block";
}
...
function agent_socket_message_handler(e) {
    ...
    if (sender == 'inactivity_end_session') {
        ...
        var translated_text = message["translated_text"];
        let agent_inactivity_text_message = message["text_message"];

        var inactivity_message = "Due to inactivity chat has ended";
        if(translated_text) {
            inactivity_message = translated_text;
        }
        close_livechat(inactivity_message, agent_inactivity_text_message);
        ...
        if (message['session_id'] == livechat_session_id) {
            ...
            chat_escalation_report_agent_message = message["text_agent_message"];
    ...
    if (sender == "agent_end_session") {
        ...
        if ("event_type" in message) {
            if (message["event_type"] == "CHAT_TRANSFERRED" && "session_id" in message) {
                ...
                count_of_unread_message++;
                maximize_chatbot();   
            } else if(message['event_type'] == "IS_AGENT_TRANSCRIPT_REQUEST"){
                hide_transcript_option();
                is_transcript_sent = true;
                hide_transcript_option();
                append_system_text_response(LIVECHAT_TRANSCRIPT_SEND_TEXT);
            }
        }
    }
}
...
function close_livechat_socket(e) {
    ...
    if (is_livechat && is_chat_socket_open) {
        ...
        if (!bot_restarted) {
            if (SHOW_WELCOME_MSG_ON_END_CHAT) {
                append_welcome_message(bot_id, bot_name, true);
            } else {
                handle_welcome_message(default_response); 
            }
...
function check_tab_focused() {
    if (document.visibilityState === 'visible' && is_livechat && is_iOS()) {

        webkit.messageHandlers.setLiveChatSessionID.postMessage({"livechat_session_id": livechat_session_id});
    }
}

function handle_welcome_message(default_response) {
    let message_list = get_message_list(default_response, RESPONSE_SENTENCE_SEPARATOR);
...
function get_transcript_message() {
    try {
         return '<p>\
                '+ LIVECHAT_GOOD_BYE_TEXT + ' <br> <br>\
                         <span>\
                    '+ LIVECHAT_CHAT_TRANSCRIPT_MAIL_TEXT +' <br>\
                    <a href="#" id ="disable-click-link-'+String(livechat_session_id)+'" \
                    onclick = "send_transcript(true)" style="color: #0254D7;">\
                        '+ LIVECHAT_CLICK_HERE_TEXT + ' \
                        </a> \
                        <div id="click-hear-svg-'+String(livechat_session_id)+'"></div>\
                </span>\
            </p>';
     }
     catch(err) {
...
function append_feedback_form(livechat_session_feedback, livechat_user_feedback) {
   ...
   try {
        ...
        enable_language_change_options();
        if(IS_TRANSCRIPT_ENABLED && is_transcript_sent == false && EASYCHAT_BOT_THEME != 'theme_4') {
            append_bot_text_response(get_transcript_message(), undefined, undefined, undefined);
        }
        if (EASYCHAT_BOT_THEME == 'theme_4') {
...
function get_feedback_form_html() {
    ...
    html += '<div class="feedback-form-button-div" style="display: flex;justify-content: flex-start;gap: 8px;flex-direction: row-reverse;"><button class="btn right" id ="feedback-submit-button"  disabled = "true" style="background-color:' + BOT_THEME_COLOR + ' !important;margin-left: 0.2em;border-radius: 30px !important;font-size: 14px !important;font-weight: 400 !important;color: white !important;cursor: pointer;text-align: center;height: 36px;line-height: 36px;padding: 0 16px;vertical-align: middle;margin-top: 0.2em;opacity:0.5; border:aliceblue;outline:none;" onclick="hide_click_hear_link();add_livechat_nps();">'+ SUBMIT_TEXT +'</button><a class="right" style="background-color: white !important;margin-left: 0.2em;color: #757575;font-size: 14px !important;text-decoration: none;margin-top: 0.9em;margin-right: 0.5em;cursor:pointer;" onclick="hide_click_hear_link();add_livechat_nps();"><b>' + LIVECHAT_FEEDBACK_CANCEL_TEXT + '</b></a></div>';

    var parent_html =
        '<div id="livechat_feedback" style="display:inline-block;"><div id="feedback_wrapper" style="box-shadow: 1px 1px 6px rgba(0, 0, 0, 0.2);color: black;width: 90%;margin: auto;margin-top: 1.5em;border-radius: 1em;padding: 1em;background-color:white;padding-right: 1.1em;height: auto !important;margin-bottom:1em;">' +
        user_input +
        html +
        "</div></div>";
                    ...
                    <div class="easychat-bot-custom-modal-body">\
                        ...
                        <div class="livechat-feeback-comment-wrapper">\
                            <textarea class="livechat-feedback-comment-box" placeholder=" ' + LIVECHAT_ADD_COMMENT_TEXT + '" col="30"\
                                id="livechat-chatbot-comment-box" onchange="save_livechat_feedback_text()"></textarea>\
                        </div>';
                        
                        if(IS_TRANSCRIPT_ENABLED && is_transcript_sent == false) {
                            html += '<div class="transcript-checkbox-div">\
                            <div class="transcript-checkbox-input-div">\
                                <input type="checkbox" id="transcript-checkbox" onchange="check_for_theme4_transcript()">\
                                <label for="transcript-checkbox">' + LIVECHAT_CHECK_BOX_TRANSCRIPT_TEXT + '</label>\
                                </div>\
                            </div>';
                        }
                     html += '<div class="easychat-bot-custom-modal-footer" style="margin-top: 4px; padding-top: 8px;">\
                        ...
                        <button class="custom-modal-submit-btn bot-close-modal" id="feedback-submit-button" disabled="true"\
                            onclick="submit_transcript();add_livechat_nps()" style="opacity: 0.5;"> ' + SUBMIT_TEXT + '</button>\
                    </div>\
                </div>\
            </div>';
    return html;
...
function hide_click_hear_link() {
    let is_disable_link = document.getElementById(`disable-click-link-${livechat_session_id}`);
    if(is_disable_link) {
        is_disable_link.classList.add("disable-link");
    }
}

function check_for_theme4_transcript() {
    is_transcript_enabled = document.getElementById("transcript-checkbox").checked;
}

function check_for_csat_transcript() {
    is_csat_transcript_enabled = document.getElementById("feedback-transcript-checkbox").checked;
}

function submit_transcript() {
    if (is_transcript_enabled || is_csat_transcript_enabled){
        send_transcript(true);
    } 
    if (is_csat_transcript_enabled){
      let message = "The transcript will be sent over mail";
      let sentence = JSON.stringify({
        message: JSON.stringify({
            text_message: message,
            ...
            path: "",
            event_type: "IS_CUSTOMER_TRANSCRIPT_REQUEST",
            is_transcript_system_message: false
        }),
        sender: "System",
        });
...
function append_file_to_customer(url, message, thumbnail_path, default_time, other_info) {
    ...
    message = message.replace("</strong>", "</b>");
    message = message.replace("<em>", "<i>");
    message = message.replace("</em>", "</i>");
    message = sanitizeHTML(message);
    
    message = easychat_linkify(message);

    message = message.replace(new RegExp('\r?\n', 'g'), '<br>');
    message = message.replace(new RegExp('\r?<table', 'g'), '<table class="easychat-bot-table" style="width:100%"');
...
function set_footer_for_livechat() {
    ...
    try {
        ...
        if (EASYCHAT_BOT_THEME == "theme_2") {
            ...
            user_input_elem.style.width = "100%";
...
function disable_sticky_intents() {
    try {
        if (EASYCHAT_BOT_THEME == "theme_3") {
            document.getElementById("easychat-bot-sticky-intent-wrapper").style.pointerEvents = "none";
        }
        else {
...
function enable_sticky_intents() {
    try {
        if (EASYCHAT_BOT_THEME == "theme_3") {
            document.getElementById("easychat-bot-sticky-intent-wrapper").style.pointerEvents = "unset";
        }
        else {
...
function paste_only_numbers(input){
