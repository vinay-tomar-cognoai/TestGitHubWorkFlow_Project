...
class MessageCluster(object):
    ...
    def send_to_msisdn(self, msisdn, service_account_location):
        """
        ...
        for it in range(0, len(self._messages)):
            ...
            if it < len(self._messages) - 1:
                ...
                response = rbm_service.send_message_with_body_and_suggestion_chip_list(
                    msisdn,
                    message.get_agent_message(),
...
class Message(object):
    ...
    def prepare_to_send(self, service_account_location):
        """
...
class StandaloneCard(Message):
    ...
    def prepare_to_send(self, service_account_location):
        ...
        if not self._file_resource_id and self._url:
            self._file_resource_id = rbm_service.upload_file(
                self._url, service_account_location, self._thumbnail_url)
        return self
...
class CarouselCard(Message):
    ...
    def prepare_to_send(self, service_account_location):
        ...
        """
        for card_content in self._card_contents:
            card_content.prepare_to_send(service_account_location)
...
class CardContent(object):
    ...
    def get_card_content_object(self):
        ...
        if self._suggestions:
            card_content['suggestions'] = get_suggestion_chip_list(
                self._suggestions)
...
class ShareLocationAction(SuggestionChip):
    ...
    def get_suggestion_chip_list(self, suggestions):
        """
        ...
        for suggestion in suggestions:
            suggestion_json_list.append(
                suggestion.get_suggestion_chip_object())
