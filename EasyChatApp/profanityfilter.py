...
class ProfanityFilter:
    def __init__(self, **kwargs):
        ...
        self._words_file = os.path.join(self._BASE_DIR, 'badwords.txt')
    ...
    def has_bad_word(self, text, bot_info_obj):
        """Returns True if text contains profanity, False otherwise."""
        return self.censor(text, bot_info_obj) != text
    ...
    def censor(self, input_text, bot_info_obj):
        """Returns input_text with any profane words censored."""
        bad_words = []
        if bot_info_obj != None:
            bad_words = json.loads(bot_info_obj.bad_keywords)
            if len(bad_words) == 0:
                bad_words = self.get_profane_words()
                bot_info_obj.bad_keywords = json.dumps(bad_words)
                bot_info_obj.save()
        else:
            bad_words = self.get_profane_words()
        res = input_text
        ...
        for word in bad_words:
            ...
            censor_str = word[0] + (self._censor_char * (len(word)-1))
            res = regex.sub(censor_str, res)

        return res


    def is_clean(self, input_text, bot_info_obj=None):
        """Returns True if input_text doesn't contain any profane words, False otherwise."""
        return not self.has_bad_word(input_text, bot_info_obj)
