...
from EasyChatApp.utils import save_audit_trail, logout_all
from EasyAssistApp.utils import save_audit_trail as save_cobrowse_audit_trail
from EasyAssistApp.models import CobrowseAgent, CobrowsingAuditTrail
from EasyAssistApp.constants import COBROWSING_LOGIN_ACTION
...
def _create_new_user(username, email, firstname, lastname):
    ...
    if email.endswith("@getcogno.ai") == False:
        SelfSignupUser.objects.create(
            firstname=firstname, lastname=lastname, username=email, is_completed=False)
        user.is_guest = True
        user.save()
...
def acs(request):
    ...
    next_url = request.session.get('login_next_url', settings.SAML2_AUTH.get(
    ...
    # if user_email.endswith("@getcogno.ai"):
    #     user_email = user_email.replace("getcogno.ai", 'getcogno.ai')
    #     user_name = user_name.replace("getcogno.ai", 'getcogno.ai')
    ...
    try:
        ...
        logout_all(user_name)
        save_audit_trail(target_user, USER_LOGGED_IN, audit_trail_data)
        try:
            cobrowse_agent = CobrowseAgent.objects.filter(user=target_user)
            if cobrowse_agent:
                cobrowse_agent = cobrowse_agent[0]
                save_cobrowse_audit_trail(
                    cobrowse_agent, COBROWSING_LOGIN_ACTION, "Login into System - SSO", CobrowsingAuditTrail)
        except Exception:
            pass
...
def signin(request):
    ...
    next_url = request.GET.get('next', settings.SAML2_AUTH.get(
        'DEFAULT_NEXT_URL', get_reverse('admin:index')))
